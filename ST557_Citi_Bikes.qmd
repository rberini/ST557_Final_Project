...
```{r}
print(citibike_2018$`start station latitude`[1], digits = 15)
```
...
### Log scale for trip duration (excluding extreme values)

```{r}
citibike_2018 |>
  filter(tripduration_min > 0 & tripduration_min < 2880) |>
  ggplot(aes(x = log10(tripduration_min))) +
  geom_density() +
  labs(title = "Distribution of Trip Duration (Log Scale)", 
       x = "Log Base 10 (Duration in minutes)", 
       y = "Density")
```
...
```{r}
write_csv(citibike_2018, "data/combined/citibike_2018_full.csv")
write_rds(citibike_2018, "data/combined/citibike_2018_full.rds")
